...
%DIF DEL ../versions/tex/ODLEDTN_v1.1.tex   Thu Dec  4 11:54:13 2025
%DIF ADD ../main.tex                        Thu Jan  8 11:55:38 2026
...
\date{\today\\
{\bf Version \DIFdelbegin \DIFdel{1.1}\DIFdelend \DIFaddbegin \DIFadd{1.2}\DIFaddend }}
...
\DIFaddbegin \item \DIFadd{v1.2 - Include designs of the LED electronics crate and overall rack layouts, as requested by review committee.
...
To house the electronics for the LI systems, two \DIFdelbegin \DIFdel{42U }\DIFdelend \DIFaddbegin \DIFadd{42~U }\DIFaddend server racks with 800~mm depth will be used. \DIFaddbegin \DIFadd{1~U, or rack unit, is a standardised measurement defined as 44.45~mm (1.75~in). }\DIFaddend The front of the server rack will be used for electrical connections and displays, and the reverse/internal will be used only for fibre routing. The server racks will include Uninterruptible Power Supplies (UPS) for safe power delivery and for power processing, to avoid issues with potential instabilities in the main power supply. Each rack will include an air conditioning unit to have a controlled temperature and remove humidity from the air, as the relative humidity in the air is expected to be above 70\%. Although specific tests on running the LED electronics in humid conditions have not been carried out, it is known that the optical switches for the laser calibration system requires lower humidity levels. In order to simplify things and remove the potential of humidity issues with the LED electronics, both server racks will be air conditioned. These systems are widely available and will be chosen closer to installation.

\DIFaddbegin \DIFadd{A preliminary arrangement of all units within the two server racks is shown in }\cref{fig:serverrack}\DIFadd{. One of these racks will also house electronics for the photogrammetry and Korean LI systems. As such, the exact space requirements from those groups is currently unknown. However, the 24~U empty space should be more than enough the accommodate those two systems. Once further information on those systems is available, this design will be further refined.
}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{ServerRacks.png}
    \caption{\DIFaddFL{Preliminary arrangement of the two 42~U server racks used to house the laser and LED electronics, along with those for the photogrammetry and Korean LI systems.}}
    \label{fig:serverrack}
\end{figure}

\DIFaddend \section{LED Monitoring}
...
\subsection{\DIFadd{Overall Crate Design}}\label{cratedesign}

\cref{fig:cratedesign} \DIFadd{shows how an assembled LED crate will look, from various angles.
}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.5\textwidth}
            ...
            \subcaption{\DIFaddFL{Top view}}
    \end{subfigure}%DIF >    
\begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateFibresideView}
            \subcaption{\DIFaddFL{Front view}}
    \end{subfigure}\\
        \begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateFibresidethreeqView}
            \subcaption{\DIFaddFL{Side view}}
    \end{subfigure}%DIF > 
\begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateRackdoorView}
            \subcaption{\DIFaddFL{Back side view}}
    \end{subfigure}
    \caption{\DIFaddFL{3D rendering of an LED electronics crate with all boards and 3D printed fibre connectors installed.}}
