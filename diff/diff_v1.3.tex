...
%DIF DEL ../versions/tex/ODLEDTN_v1.2.tex   Thu Jan  8 21:01:13 2026
%DIF ADD ../main.tex                        Thu Feb 12 15:53:34 2026
...
\date{\today\\
{\bf Version \DIFdelbegin \DIFdel{1.2}\DIFdelend \DIFaddbegin \DIFadd{1.3}\DIFaddend }}
...
\item v1.1 - Add results of testing for untriggered LED pulses.
\item v1.2 - Include designs of the LED electronics crate and overall rack layouts, as requested by review committee.
\DIFaddbegin \item \DIFadd{v1.3 - Add section on gate driver selection
...
\subsubsection{Switch Selection}\DIFaddbegin \label{switchselection}
\DIFaddend 
...
\subsubsection{\DIFadd{Gate Driver Selection}}

\DIFadd{The EPC8004 is a suitable choice for a switch but to fully utilise this device we need a gate driver that can output a suitable current to drive the GaNFET. The specification required a sub-1~ns pulse, which means a combined rise and fall time has to be less than 1~ns to achieve said pulse width. Another requirement was for the component to be readily available and not to be end-of-life.
Through market research, the Texas Instruments LMG line was identified as a good fit for these requirements. They offered multiple devices in the lineup, listed in }\cref{tab:LMGFamily}\DIFadd{, and the LMG1020 was the most suitable based on number of output channels and timing features. The LMG1020 driver was ordered for use with pulser board prototypes, and was found to drive various switches well, as described in }\cref{switchselection}\DIFadd{. For these reasons, the LMG1020 driver was chosen.
}

\begin{table}[h]
  \centering
  \caption{\DIFaddFL{Texas Instrument LMG family of devices}}
  \label{tab:LMGFamily}
  \begin{tabular}{l | c | c | c}
    \toprule
    \DIFaddFL{Part number }& \DIFaddFL{Output Channels }& \DIFaddFL{Rise Time }[\DIFaddFL{ps}] & \DIFaddFL{Fall Time }[\DIFaddFL{ps}] \\
    \midrule
    \DIFaddFL{LMG1020    }& \DIFaddFL{1      }& \DIFaddFL{375 }& \DIFaddFL{350     }\\
    \DIFaddFL{LMG1210    }& \DIFaddFL{2      }& \DIFaddFL{500 }& \DIFaddFL{500    }\\
    \DIFaddFL{LMG1025   }& \DIFaddFL{1      }& \DIFaddFL{650 }& \DIFaddFL{850     }\\
    \bottomrule
  \end{tabular}
\end{table}
\DIFaddend 
...
\subsection{Untriggered Emission Test}\label{sec:untriggered}

It is important to ensure that all LED pulses, and therefore injections into the HK tank, come only when requested. If errant pulses occur without a trigger signal being sent, these could be injected without associated calibration tags, and potentially be mistaken for physics events.

In order to ensure that this does not happen with the developed electronics, the same procedure as was applied for the untriggered laser emission tests in \cite{TN91} was followed. The pulser board was set up as usual, and powered on via the motherboard, but no signal to pulse sent. The board was connected to 181~m of FP400URT fibre, which in turn was connected to a PMT. To ensure that the PMT gain and threshold on the oscilloscope were set correctly, a set of pulses were first triggered for calibration purposes. This can be seen in \cref{fig:singlepulse}, and also serves to show what should be expected on the oscilloscope if errant pulses are observed. 
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{ledtestpulsereal.png}
    \caption{Scope trace and statistics for `calibration' pulses before performing the untriggered LED emission test.}
    \label{fig:singlepulse}
\end{figure}
Once the triggering had been disabled, the oscilloscope was re-zeroed, and the test setup left for seven days. With the scope set to infinite persistence, any signal the scope triggered on would remain, and the measurement table would show a count of how many separate signals there had been.

A screenshot of the oscilloscope screen after being left for a week is given in \cref{fig:nopulse}.
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{Lednopulsenoise.png}
    \caption{Scope trace and statistics after one week of not triggering the LED. The single detected signal is noise.}
    \label{fig:nopulse}
\end{figure}
This shows that one signal was observed in this period, however it can clearly be seen from the ringing structure that this is simply noise. The optics lab in which these measurements are carried out is not completely quiet, and these signals will occur occasionally. As this is the only signal that was observed, it is clear that no errant pulses were observed from the LED. 
...
To house the electronics for the LI systems, two 42~U server racks with 800~mm depth will be used. 1~U, or rack unit, is a standardised measurement defined as 44.45~mm (1.75~in). The front of the server rack will be used for electrical connections and displays, and the reverse/internal will be used only for fibre routing. The server racks will include Uninterruptible Power Supplies (UPS) for safe power delivery and for power processing, to avoid issues with potential instabilities in the main power supply. Each rack will include an air conditioning unit to have a controlled temperature and remove humidity from the air, as the relative humidity in the air is expected to be above 70\%. Although specific tests on running the LED electronics in humid conditions have not been carried out, it is known that the optical switches for the laser calibration system requires lower humidity levels. In order to simplify things and remove the potential of humidity issues with the LED electronics, both server racks will be air conditioned. These systems are widely available and will be chosen closer to installation.

A preliminary arrangement of all units within the two server racks is shown in \cref{fig:serverrack}. One of these racks will also house electronics for the photogrammetry and Korean LI systems. As such, the exact space requirements from those groups is currently unknown. However, the 24~U empty space should be more than enough the accommodate those two systems. Once further information on those systems is available, this design will be further refined.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{ServerRacks.png}
    \caption{Preliminary arrangement of the two 42~U server racks used to house the laser and LED electronics, along with those for the photogrammetry and Korean LI systems.}
...
\subsection{Overall Crate Design}\label{cratedesign}

\cref{fig:cratedesign} shows how an assembled LED crate will look, from various angles.

\begin{figure}[h!]
    \centering
    \begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateTopView}
            \subcaption{Top view}
    \end{subfigure}%   
\begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateFibresideView}
            \subcaption{Front view}
    \end{subfigure}\\
        \begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateFibresidethreeqView}
            \subcaption{Side view}
    \end{subfigure}%
\begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{FullCrateRackdoorView}
            \subcaption{Back side view}
    \end{subfigure}
    \caption{3D rendering of an LED electronics crate with all boards and 3D printed fibre connectors installed.}
    \label{fig:cratedesign}
\end{figure}
