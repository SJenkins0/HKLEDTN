...
%DIF DEL ../versions/tex/ODLEDTN_v1.tex   Wed Oct  8 14:52:00 2025
%DIF ADD ../main.tex                      Thu Dec  4 11:53:50 2025
...
\DIFaddbegin \date{\today\\
{\bf \DIFadd{Version 1.1}}}
...
\DIFaddbegin \item \DIFadd{v1.1 - Add results of testing for untriggered LED pulses.
...
The LED board was set at 12~V, the current highest voltage output. The fibre was first connected to the PMT, to confirm that the pulse width was sub-10~ns. The power was then measured by the power meter. From this test, the maximum recorded light yield was found to be $2.4\times10^7$ photons per pulse. Further data was taken to investigate the dynamic range associated with changing pulse width and voltage, however in migrating lab PCs this has been lost, and will be retaken with the most recent versions of the hardware. Despite this, these numbers show that the system is capable of producing a large number of photons per pulse (ppp). In \cref{sec:collimator_studies} an output of $2\times10^7$ ppp was shown to yield up to 86\% saturation in the barrel region\DIFaddbegin \DIFadd{.
}

\subsection{\DIFadd{Untriggered Emission Test}}\label{sec:untriggered}

\DIFadd{It is important to ensure that all LED pulses, and therefore injections into the HK tank, come only when requested. If errant pulses occur without a trigger signal being sent, these could be injected without associated calibration tags, and potentially be mistaken for physics events.
}

\DIFadd{In order to ensure that this does not happen with the developed electronics, the same procedure as was applied for the untriggered laser emission tests in \mbox{%DIFAUXCMD
\cite{TN91} }\hskip0pt%DIFAUXCMD
was followed. The pulser board was set up as usual, and powered on via the motherboard, but no signal to pulse sent. The board was connected to 181~m of FP400URT fibre, which in turn was connected to a PMT. To ensure that the PMT gain and threshold on the oscilloscope were set correctly, a set of pulses were first triggered for calibration purposes. This can be seen in }\cref{fig:singlepulse}\DIFadd{, and also serves to show what should be expected on the oscilloscope if errant pulses are observed. 
}\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{ledtestpulsereal.png}
    \caption{\DIFaddFL{Scope trace and statistics for `calibration' pulses before performing the untriggered LED emission test.}}
    \label{fig:singlepulse}
\end{figure}
\DIFadd{Once the triggering had been disabled, the oscilloscope was re-zeroed, and the test setup left for seven days. With the scope set to infinite persistence, any signal the scope triggered on would remain, and the measurement table would show a count of how many separate signals there had been.
}

\DIFadd{A screenshot of the oscilloscope screen after being left for a week is given in }\cref{fig:nopulse}\DIFadd{.
}\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{Lednopulsenoise.png}
    \caption{\DIFaddFL{Scope trace and statistics after one week of not triggering the LED. The single detected signal is noise.}}
    \label{fig:nopulse}
\end{figure}
\DIFadd{This shows that one signal was observed in this period, however it can clearly be seen from the ringing structure that this is simply noise. The optics lab in which these measurements are carried out is not completely quiet, and these signals will occur occasionally. As this is the only signal that was observed, it is clear that no errant pulses were observed from the LED}\DIFaddend . 
